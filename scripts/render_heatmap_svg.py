...
def render_heatmap_svg(json_path=None, output_svg=None):
    ...
    # Summary Footer
    svg.append('  <g transform="translate(24, 192)" class="font-mono" font-size="11" fill="#8B949E">')
    svg.append(f'    <text fill="#F8FAFC" font-weight="700">{total} <tspan font-weight="400" fill="#8B949E">contributions in the last year</tspan></text>')
    svg.append(f'    <text x="240" fill="#8B949E">• Streak: <tspan fill="#38BDF8" font-weight="700">{streak}d</tspan> • Longest: <tspan fill="#A855F7" font-weight="700">{longest}d</tspan></text>')
    svg.append('  </g>')

    # Legend
    svg.append('  <g transform="translate(680, 182)" class="font-mono" font-size="9" fill="#8B949E">')
    svg.append('    <text x="0" y="9">Less</text>')
    for i, col in enumerate(PALETTE):
        svg.append(f'    <rect x="{30 + i*13}" y="0" width="10" height="10" rx="2" fill="{col}" />')
    svg.append('    <text x="100" y="9">More</text>')
    svg.append('  </g>')
